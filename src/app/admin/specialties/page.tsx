...
export default function SpecialtiesPage() {
	...
	const handleDialogSubmit = async (data: {
		...
		if (editId) {
			...
			const success = await addSpecialty(
				data.name,
				data.showOnCall ?? true,
				data.hasResidency ?? false
			);
			if (success) {
				setDialogDefaultValues(null);
			}
									...
									<TableRow key={spec.id}>
										...
										<TableCell>
											<Switch
												...
												onCheckedChange={() =>
													handleToggleShowOnCall(spec.id, spec.showOncall)
												}
					...
					{loading ? (
						...
						<div className="flex flex-col gap-4 p-2">
							{specialties.map((spec, idx) => (
								<div
									key={spec.id}
									className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm p-4"
								>
				...
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							...
							<span className="font-semibold">"{specialtyToDelete?.name}"</span>
