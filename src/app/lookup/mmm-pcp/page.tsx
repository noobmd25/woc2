...
export default function MMMPcpLookupPage() {
  ...
  const [results, setResults] = useState<
    { name: string; medical_group: string }[]
  >([]);
  ...
  const groupColors = useMemo(() => {
    ...
    const unique = Array.from(
      new Set(results.map((r) => r.medical_group)),
            ...
            {sorted.map((r, idx) => (
              ...
              >
                <div className="flex items-start justify-between mb-1">
                  ...
                  <span
                    className={`ml-2 inline-flex items-center rounded-md px-2 py-0.5 text-xs font-semibold ring-1 ring-inset ${groupColors[r.medical_group] || "bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-200"} ring-black/0`}
                  >
                    {r.medical_group}
                  </span>
                </div>
