...
import {
	...
	SECOND_PHONE_PREFS,
	SPECIALTIES,
	type SecondPhonePref,
} from "@/lib/constants";
import { toLocalISODate } from "@/lib/schedule-utils";
import { resolveDirectorySpecialty } from "@/lib/specialtyMapping";
import { type Provider } from "@/lib/types/provider";
import { type MiniCalendarEvent } from "@/lib/types/schedule";
...
export default function SchedulePage() {
	...
	const [secondPref, setSecondPref] = useState<SecondPhonePref>(
		SECOND_PHONE_PREFS.NONE
	);
	...
	const onDatesSet = useCallback((dateInfo: any) => {
		...
		setVisibleRange((prev) => {
			if (
				prev &&
				prev.start.getTime() === nextStart &&
				prev.end.getTime() === nextEnd
			) {
				return prev;
			}
	...
	const canEdit = useMemo(
		() => role === ROLES.ADMIN || role === ROLES.SCHEDULER,
		[role]
	);
	const isIMWithoutPlan = useMemo(
	...
	useEffect(() => {
		...
		const timeoutId = setTimeout(() => {
			loadEntries(
				toLocalISODate(visibleRange.start),
				toLocalISODate(visibleRange.end),
				specialty,
				plan
			).then(() => {
	...
	const miniCalendarEvents: MiniCalendarEvent[] = useMemo(() => {
		...
		return entries
			.filter(
				(entry) =>
					entry.on_call_date >= startDateStr && entry.on_call_date <= endDateStr
			)
			.map((entry) => ({
	...
	useEffect(() => {
		if (isModalOpen && selectedModalDate) {
			const modalDate = new Date(selectedModalDate + "T12:00:00");
			setMiniCalendarDate(
				new Date(modalDate.getFullYear(), modalDate.getMonth(), 1)
			);
		}
	...
	useEffect(() => {
		...
		const targetName =
			secondPref === SECOND_PHONE_PREFS.RESIDENCY
				? `${baseSpec} Residency`
				: `${baseSpec} PA Phone`;
		...
		if (provider?.phone_1) {
			...
			setSecondPhone(
				`No ${secondPref === SECOND_PHONE_PREFS.RESIDENCY ? SECOND_PHONE_PREFS.RESIDENCY : SECOND_PHONE_PREFS.PA} phone registered for this service.`
			);
			setSecondSource(null);
		}
		...
		if (isModalOpen && editingEntry) {
			...
			if (cover && editingEntry.covering_provider) {
				...
				const provider = allProviders.find(
					(p) => p.name === editingEntry.covering_provider
				);
		...
		(info: any) => {
			...
			if (api) {
				...
				if (
					...
					toast.error(
						"You can only add events for the current visible month. Move to that month to add events."
					);
	...
	const handleEventClick = useCallback(
		(clickInfo: any) => {
			...
			if (entry) {
				...
				setSecondPref(
					entry.second_phone_pref === SECOND_PHONE_PREFS.AUTO
						? SECOND_PHONE_PREFS.NONE
						: (entry.second_phone_pref as SecondPhonePref) ||
								SECOND_PHONE_PREFS.NONE
				);
	...
	const handleModalSubmit = useCallback(async () => {
		...
		try {
			const baseEntry = {
				...
				specialty,
				healthcare_plan:
					specialty === SPECIALTIES.INTERNAL_MEDICINE ? plan : null,
				show_second_phone: secondPref !== SECOND_PHONE_PREFS.NONE,
				second_phone_pref:
					secondPref === SECOND_PHONE_PREFS.NONE
						? ("auto" as const)
						: secondPref,
				cover: coverEnabled,
	...
	const handleConfirmedDelete = useCallback(async () => {
		...
		try {
			...
			const success = await deleteEntryByDateAndProvider(
				date,
				provider,
				specialty,
				plan
	...
	const handleClearConfirmed = useCallback(async () => {
		...
		const { success, count } = await clearMonth(
			startOfMonth,
			startOfNextMonth,
			specialty,
			plan
	...
	const renderEventContent = useCallback(
		(eventInfo: EventContentArg) => {
			...
			const currentProvider = eventInfo.event.title
				.replace(/^Dr\. /, "")
				.replace(/ \(Cover\)$/, "")
				.trim();
			const hasSecondPhone =
				entry?.show_second_phone && entry?.second_phone_pref !== "auto";
		...
		<div className="container mx-auto px-2 sm:px-4 py-3 sm:py-6 max-w-4xl overflow-x-hidden">
				...
				<div className="flex flex-row justify-between items-center p-3 sm:p-4 md:p-6 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-gray-50 to-white dark:from-gray-800 dark:to-gray-900">
					<div>
						<h2 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900 dark:text-white break-words">
							...
							{specialty === SPECIALTIES.INTERNAL_MEDICINE && plan && (
								<span className="text-blue-600 dark:text-blue-400 ml-2 text-base sm:text-lg">
									({plan})
								</span>
							)}
						</h2>
				...
				>
					...
					{entriesLoading && (
						<div className="absolute inset-0 flex flex-col items-center justify-center bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm rounded-lg z-10">
							<LoadingSpinner size="lg" />
							<span className="mt-3 text-gray-700 dark:text-gray-300 text-xs sm:text-sm font-medium">
								Loading schedule...
							</span>
						</div>
					)}
			...
			<div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-2 sm:gap-4 mt-3 sm:mt-4">
				...
				>
					...
					<span className="sm:hidden">Clear Month</span>
				</button>
			</div>
						...
						<AlertDialogDescription>
							{eventToDelete && (
								<>
									...
									<strong>{eventToDelete.provider}</strong> from{" "}
									<strong>
										{new Date(
											eventToDelete.date + "T12:00:00"
