          ...
          <FullCalendar
            ...
            eventContent={(event: EventContentArg) => {
              try {
                ...
                const sameProviderEvents = events.filter(e => e.title === event.event.title);
                ...
                return (
                  <div
                    ...
                    onClick={() => {
                      // Convert event.event.startStr to local date string at midnight (Eastern Time, UTC-5, no DST)
                      const start = new Date(event.event.startStr);
                      const easternDate = new Date(Date.UTC(start.getUTCFullYear(), start.getUTCMonth(), start.getUTCDate(), 5));
                      const localDateStr = easternDate.toISOString();
                      setSelectedModalDate(localDateStr);
                      setIsModalOpen(true);
                      setIsEditing(true);
                      ...
                      if (resident?.phone_number) {
                        ...
                        setResidentPhone('No residency phone registered for this service.');
                      }
                      if (providerInputRef.current) {
                        providerInputRef.current.value = currentProvider;
                      }

                      const multiDayToggle = document.getElementById('multi-day-toggle') as HTMLInputElement;
                      if (multiDayToggle) {
                        multiDayToggle.checked = sameProviderEvents.length > 1;
                        const miniCalendar = document.getElementById('multi-day-calendar');
                        if (miniCalendar) {
                          miniCalendar.classList.toggle('hidden', sameProviderEvents.length <= 1);
                        }
                      }
                    }}
                  >
            ...
            dateClick={(arg: any) => {
              if (specialty === 'Internal Medicine' && !plan) {
                toast.error('Please select a healthcare plan before assigning a provider.');
                return;
              }
              ...
              if (
                ...
                return; // prevent click outside current month
              }
              // Manually offset to Eastern Time (UTC-5, no DST)
              const easternDate = new Date(Date.UTC(clickedDate.getFullYear(), clickedDate.getMonth(), clickedDate.getDate(), 5));
              const localDateStr = easternDate.toISOString().split('T')[0];
              setSelectedModalDate(`${localDateStr}T00:00:00`);
              setIsModalOpen(true);
              setIsEditing(false);
            }}
          />
              ...
              <div className="grid grid-cols-7 gap-1 text-center text-xs text-gray-500 dark:text-gray-400">
                {(() => {
                  ...
                  for (let day = 1; day <= daysInMonth; day++) {
                    // Remove Date.UTC to avoid timezone offset issues
                    ...
                    const isTaken = assignedEvents.length > 0;
                    const isSelected = selectedAdditionalDays.includes(dateStr);

                    // Determine if the taken date is by the currently selected provider
                    const isTakenByCurrentProvider = assignedEvents.some(
                      e => e.title === `Dr. ${providerInputRef.current?.value.trim()}`
                    );

                    // Highlight the currently selected modal date in soft green
                    const isPrimaryDate =
                      selectedModalDate &&
                      dateStr === selectedModalDate.split('T')[0];

                    // Debug: log rendering of each date cell
                    console.log('Rendering date cell for:', { day, dateStr, selectedModalDate });

                    let cellClasses = 'p-1 rounded text-sm text-center ';
                    if (isTakenByCurrentProvider) {
                      cellClasses += 'bg-blue-200 dark:bg-blue-600 text-white font-semibold cursor-not-allowed';
                    } else if (isTaken) {
                      cellClasses += 'bg-gray-200 dark:bg-gray-700 text-gray-400 cursor-not-allowed';
                    } else if (isSelected) {
                      cellClasses += 'bg-blue-400 text-white font-semibold cursor-pointer';
                    } else if (isPrimaryDate) {
                      cellClasses += 'bg-green-100 text-green-900 font-semibold cursor-pointer';
                    } else {
                    ...
                    calendarCells.push(
                      <div
                        ...
                        onClick={(e: React.MouseEvent<HTMLDivElement>) => {
                          const isProviderMatch = assignedEvents.some(
                            e => e.title === `Dr. ${providerInputRef.current?.value.trim()}`
                          );
                          if (isTaken && !isProviderMatch) return;
                          setSelectedAdditionalDays(prev => {
                            if (selectedModalDate && dateStr === selectedModalDate.split('T')[0]) {
                              return prev; // Skip adding primary date again
                            }
