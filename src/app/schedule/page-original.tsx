...
export default function SchedulePage() {
  ...
  useEffect(() => {
    if (isModalOpen) {
      ...
      // Initialize mini calendar to current month when modal opens
      if (selectedModalDate) {
        const modalDate = parseLocalYMD(selectedModalDate);
        setMiniCalendarDate(
  ...
  }, [isModalOpen, secondPref, specialty, directory, selectedModalDate]);

  // Load mini calendar events when modal opens or month changes
  useEffect(() => {
    const loadMiniCalendarEvents = async () => {
      ...
      const startStr = toLocalISODate(startOfMonth);
      const endStr = toLocalISODate(endOfMonth);
        ...
        >
          ...
          >
            ...
            <div className="px-4 sm:px-6 py-4 space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    // Clear selected additional days when provider changes
                    // This forces a re-evaluation of date availability
                    setSelectedAdditionalDays([]);
                  }}
                  onKeyDown={(e) => {
              ...
              {/* Mini Calendar for Additional Days Selection */}
              {!isEditing && (
                <div>
                      ...
                      <FullCalendar
                        ...
                        dayCellClassNames={(arg) => {
                          ...
                          const isAdditionalSelected =
                            selectedAdditionalDays.includes(dateStr);
                          ...
                          const hasOtherProvider =
                            existingEvent &&
                            existingEvent.provider !== currentProviderName;
                          const hasPendingEntry = pendingEntries.some(
                            (entry) =>
                              entry.on_call_date === dateStr &&
                              entry.provider_name !== currentProviderName &&
                    ...
                    <div className="mt-3 space-y-2">
                      {selectedAdditionalDays.length > 0 && (
                        <div className="text-xs text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 p-2 rounded-md">
                          <strong>
                            Additional days selected (
                            {selectedAdditionalDays.length}):
                          </strong>{" "}
