...
export async function POST(req: Request) {
  try {
    ...
    if (!actingProfile || actingProfile.role !== 'admin' || actingProfile.status !== 'approved') {
      const res = new NextResponse('Forbidden', { status: 403, headers: { 'Cache-Control': 'no-store' } });
      commit(res); return res;
    }
    ...
    if (rateLimit(`admin-force-reset:${session.user.id}`) || rateLimit(`admin-force-reset-target:${email}`)) {
      const res = json({ ok: false, error: 'Too many requests. Please try again later.' }, { status: 429 });
      commit(res); return res;
    }
    ...
    const fromRawSource = process.env.RESEND_FROM || process.env.APPROVAL_EMAIL_FROM || "Who's On Call <onboarding@resend.dev>";
    if (!supabaseUrl || !serviceKey) {
      const res = json({ ok: false, error: 'Server not configured (missing SUPABASE env).' }, { status: 500 });
      commit(res); return res;
    }
    if (!resendKey) {
      const res = json({ ok: false, error: 'Server not configured (missing RESEND_API_KEY).' }, { status: 500 });
      commit(res); return res;
    }
    ...
    let origin: string;
    try {
      origin = process.env.APP_BASE_URL || process.env.NEXT_PUBLIC_SITE_URL || new URL(req.url).origin;
    } catch {
    ...
    const admin = createSupabaseAdminClient(supabaseUrl, serviceKey, { auth: { autoRefreshToken: false, persistSession: false } });
    ...
    if (error || !data?.properties?.action_link) {
      // Best-effort log
      try { await supabase.from('signup_errors').insert({ email, error_text: `generate_link_failed: ${(error?.message || 'unknown')}`.slice(0,1000), context: { stage: 'force_reset_generate', redirectTo } }); } catch {}
      const res = json({ ok: false, error: error?.message || 'Failed to generate reset link' }, { status: 400 });
      commit(res); return res;
    }
    ...
    if (!from) {
      const res = json({ ok: false, error: 'Invalid FROM address. Check RESEND_FROM/APPROVAL_EMAIL_FROM.' }, { status: 400 });
      commit(res); return res;
    }
    ...
    if (sendRes?.error) {
      try { await supabase.from('signup_errors').insert({ email, error_text: `resend_send_failed: ${sendRes.error.message}`.slice(0,1000), context: { stage: 'force_reset_send', from } }); } catch {}
      const res = json({ ok: false, error: sendRes.error.message || 'Failed to send email' }, { status: (sendRes.error.statusCode || 502) });
      commit(res); return res;
    }

    try { await supabase.from('signup_errors').insert({ email, error_text: 'force_reset_sent', context: { stage: 'force_reset', from } }); } catch {}

    const res = json({ ok: true, email });
    commit(res); return res;
  } catch (e: any) {
    const { commit } = await getServerSupabase();
    const res = json({ ok: false, error: e?.message || 'Unexpected error' }, { status: 500 });
    commit(res); return res;
  }
}
