...
const ScheduleModal = memo(
	({
		...
		const providerInputValue = currentProvider
		...
		const [coveringProviderSearchQuery, setCoveringProviderSearchQuery] =
			useState("");
		const coveringProvider = providers.find((p) => p.id === coveringProviderId);
		const coveringProviderInputValue = coveringProvider
			? coveringProvider.name
			: coveringProviderSearchQuery;
		...
		const availableCoverProviders = providers.filter(
		...
		return (
			<Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
				<DialogContent data-tour="add-entry-modal">
					<DialogHeader>
						<DialogTitle>
						...
						<div className="flex items-center gap-2">
							<Checkbox
								...
								onCheckedChange={(checked) =>
									onCoverEnabledChange(checked as boolean)
								}
						...
						{coverEnabled && (
							<div>
								...
								<ProviderSearch
									...
									selectedProvider={
										coveringProvider ? coveringProvider.name : ""
									}
									loading={loading}
								/>
							</div>
						...
						{!isEditing && (
							<div className="flex items-center gap-2">
								<Checkbox
									...
									onCheckedChange={(checked) =>
										setShowMultiDaySelector(checked as boolean)
									}
